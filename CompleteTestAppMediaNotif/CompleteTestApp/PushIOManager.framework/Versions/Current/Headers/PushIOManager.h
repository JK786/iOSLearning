...
//  PushIOManager
//
//  Copyright © 2009-2016 Oracle. All rights reserved.
//
...
/**
 ...
 NOTE: To play the sound, application must link in the AVFoundation framework.
 @param audioFileName  The name of an audio file has been passed in the push.
...
/**
 ...
 If application pass back NO, then an alert will be presented with the given alert text and no title.
...
/**
 ...
 If application pass back NO, then an SDK will updated the application badge with the badge value.
...
/**
 ...
 @param urlString A URL has been sent in the push.
...
/**
 The push included a token string for further processing.
...
/**
 @deprecated This method is deprecated starting in version 6.31.0
...
/**
 Help setting configuration type of SDK to use.
...
/**
 ...
 - PIOErrorCodeMaximumRetryReached: Maximum retrial for the web communication reached.
...
/**
 ...
 @param response detailed message of completion.
 */
...
/**
 ...
 @param error    placeholder to populate the error code/reason when operation completed.
 @param messages List of messages fetch. If messages are empty check the error for more details.
 */
...
/**
 A `PushIOManagerDelegate` implementation ensures to get called back with the progress/completion of registration. Let application know if Newsstand constant is available.
 */
...
/**
 ...
 @param error      placeholder to populate the error code/reason if registration failed.
...
/**
 A `PushIOManager` provides primary interface for application to interact with PushIO SDK. Once registration is completed all Push related operations unlocked to use . i.e.: inApp messaging, Notification preferences, categories handling, engagement reporting etc.
 */
...
/**
 Event properties (dictionary) provided by application and becomes part of every event created.
 */
 ...
 */
@property (nonatomic, assign) BOOL sdkRegistered;
...
/**
 When set the value PIOConfigTypeRelease SDK performs the 'release type API key' validation.
 When set the value PIOConfigTypeDebug SDK performs the 'debug type API key' validation.
 When set the value PIOConfigTypeNone SDK tries to determine the current config and read the configurations from respective pushio_config_debug.json or pushio_config.json and performs the APIKey validation validation.
 */
...
/**
 After application configured successfully, it has to register to let server know about the application's metadata/properties.
 
 @param error             placeholder to populate the error code/reason if registration fail.
 @param completionHandler callback to notify when registration complete with server.
 
 @return TRUE if register event created and stored successfully, FALSE otherwise.
 @warning return value ensures the registration event creation locally, and completion handler gets called when sync of registration happen with server.
 */
...
/**
 Asks user for permissions of different all notifications types i.e.: Sound/Badge/Alert types.
 If readyForRegistrationCompHandler is not set, then provided completionHandler is assigned to it. To let application have access when SDK receive deviceToken.

 @param completionHandler callback to notify when asking for notification permission completed.
...
/**
 ...
 @warning return value ensures the unregister event creation locally, and completion handler gets called when sync of unregister happen with server.
 */
...
/**
 Creates event and adds extra event properties provided by application through setting the proper values in globalEventProperties and globalEventPropertiesCallback. Stores created event in SDK managed persistence storage which is synced with server.
...
/**
 Creates event and adds extra event properties provided in parameter 'properties' and through setting the proper values in globalEventProperties and globalEventPropertiesCallback. Stores created event in SDK managed persistence storage which is synced with server.
...
/**
 Trigger the engagement event for the provided engagement metric type.
...
/**
 Remove a preference identified by provided key. Preferences must be redeclared before assign again.
...
/**
 Removes all preferences. Preferences must be redeclared before assign again.
 */
...
/**
 @warning This override will make it inconsistent IAM and RichPush. Don't use this untill consequences are fully analysed.
 */
@property (nonatomic, strong) NSString *overrideURLScheme;

      
/**
 @unavailable This method is deprecated starting in version 6.31.0
...
/**
 Asks user for permissions of provided notifications types i.e.: Sound/Badge/Alert types.
 If readyForRegistrationCompHandler is not set, then provided completionHandler is assigned to it. To let application have access when SDK receive deviceToken.
 ...
 */
-(void) registerForNotificationAuthorizations:(UNAuthorizationOptions)authOptions completionHandler:(PIOCompletionHandler)completionHandler;
 ...
 */
-(void) registerForNotificationTypes:(UIUserNotificationType)notificationType completionHandler:(PIOCompletionHandler)completionHandler;
